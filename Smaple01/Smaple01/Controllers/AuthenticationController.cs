...
namespace Smaple01.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        {
            //we don't have a user DB or table so can't check the passed-through
            //for demo purposes, we assume the credentials are valid
            //return a new CityInfoUser. values would normally come from your user DB/table. but now we assume there is only one user(with id = 1, ...)

            return new CityInfoUser(1, userName ?? "", "Ali", "Akbari", "NewYork");
        }
    }
}
